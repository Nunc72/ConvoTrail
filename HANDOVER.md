...
_Last updated: 2026-04-24, v0.0.58_
...
- v0.0.58 — Pinch-to-zoom was still blocked because page viewport had user-scalable=no + maximum-scale=1 + minimum-scale=1, which on iOS outright disables multi-finger gesture events (both touchmove-2-fingers AND WebKit's gesturestart/change/end). Dropped those from the meta — iOS input-focus auto-zoom is already prevented by our 16px input font-size media query, so the usual reason to disable user-scalable doesn't apply. Now native pinch works inside the iframe newsletter, and the image preview's gesture handlers actually fire. Pull-to-sync: listeners moved from the inner scrollable div to the OUTER non-scrolling wrapper (overflow:hidden). On Convo tab, iOS' scroll-momentum on the inner div was swallowing touchmove updates even with {passive:false}; binding on the non-scrolling parent bypasses that entirely.
- v0.0.57 — Mobile gets a dedicated Sync button in the top bar (between user menu and +) — always works, pull-to-sync is the shortcut. Pull-to-sync itself rewritten once more: native addEventListener with {passive:false} on the scrollable div, preventDefault() while actively pulling — this takes ownership of the gesture from iOS so touchmove keeps firing reliably on long scrollable lists (Convo tab) where iOS' own rubber-band handler used to swallow them. Pinch-to-zoom: image preview modal now uses WebKit `gesturestart/change/end` (which fire even with page-level `user-scalable=no`) with a touchmove-based Android fallback; iframe newsletter explicitly advertises `user-scalable=yes, maximum-scale=5` in its own viewport so native pinch works inside the frame.
- v0.0.56 — Pull-to-sync on long lists (Convo tab) finally works. Root cause was that the touch listeners were attached to the scrollable inner element, which on scrollable content iOS grants to its own native overscroll handling, silently suppressing the React/DOM touchmove updates after a few frames. Fix: moved the handlers to the outer non-scrolling wrapper (which has overflow:hidden) via React props — events bubble up from the inner div and we always see them. Widened the "near top" fuzz to 10 px and simplified the abort to: only reset pullDist when dy ≤ 0, only fully abort when scrollTop > 20. Also dropped the success "Mail synced" toast (the "Syncing…" indicator + fresh mail are self-evident) — only errors still toast. Image preview: mobile no longer cycles zoom on tap — only pinch adjusts. Desktop still cycles on click / trackpad tap as before.
